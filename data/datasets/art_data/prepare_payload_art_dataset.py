...
# VAE
with open(f'train_payloads_vae.txt', 'w') as f:
    for cls_idx, p in train:
        f.write(p + '\n')
with open(f'test_payloads_vae.txt', 'w') as f:
    for cls_idx, p in test:
        f.write(p + '\n')

# Classifier
with open(f'train_payloads_cls.txt', 'w') as f:
    for cls_idx, p in train:
        f.write(f"{cls_idx}\t{p}\n")
with open(f'test_payloads_cls.txt', 'w') as f:
    for cls_idx, p in test:
        f.write(f"{cls_idx}\t{p}\n")

# GAN
with open(f'train_payloads_gan.txt', 'w') as f:
    for cls_idx, p in train:
        f.write(f"0\t{p}\n")
with open(f'test_payloads_gan.txt', 'w') as f:
    for cls_idx, p in test:
        f.write(f"0\t{p}\n")

# Skkiped
with open(f'skip_payloads.txt', 'w') as f:
    for t in skip:
        f.write(f"{t}\n")

# Technique, Class, Count
with open(f'technique_payloads_cls_c.txt', 'w') as f:
